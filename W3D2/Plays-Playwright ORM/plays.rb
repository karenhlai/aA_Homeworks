...
class Play
  ...
  def self.find_by_playwright(name)
    ...
    by_playwright = PlayDBConnection.instance.execute(<<-SQL, playwright.id)
      SELECT
        *
      FROM
        plays
...
class Playwright 
  ...
  def self.find_by_name(name)
    person = Playwright.instance.execute(<<-SQL, name)
      SELECT 
        *
      FROM
        playwrights
  ...
  def initialize(options) #initlaize here
    @id = options['id']
    @name = options['name']
  ...
  def get_plays #return all plays written by playwright 
    raise '#{self} not in database' unless self.id
